...
int main()
{
    ...
    //this takes the vector of words we used to set up our tree and searches for them. if the tree is implemented properly
    //then all these words must be found in the tree as it is how we built it. additionally, this also checks if our search process
    //works properly. if this step does not complete, then either our search or our tree must be incorrect
    for (int j=0; j<words.size();j++)
    {
        if(b->is_word_in_tree(words[j])==false)
        {
            //well, if we have an issue we should write it to the console
            cout<<"A word to set up our tree is not found. Please check your code and try again.";
            return 0;
        }
        else
        {
    ...
    //how do we test to see if it is searching properly? well, the only words we KNOW are in the tree are the ones we put in there, right?
    //so to see if it is searching properly, we need to see if our function can find the words already inside the tree
    //however, we have already done this in the first test written above
    //but wait, we searched our tree assuming our search function would tell us if we searched for a word that wasn't there
    //let's make sure our search function can identify a word we KNOW isn't in the tree
    string test ="yyyyyyyyyyy";
    if (b->is_word_in_tree(test)==false)
    {
        cout<<"Our search has recognized an invalid word.";
    }
    else
    {
        cout<<"There is an issue with our search function in particular";
    }
    //here we have created a test word with length greater than the words in our tree. if there is no issue finding this word in
    //our tree, then we know our search mechanism has erred.
